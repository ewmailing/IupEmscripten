...
* \ingroup linefile */
IUP_EXPORTI IlineFile* iupLineFileOpen(const char* filename);

/** Closes the file.
* \ingroup linefile */
IUP_EXPORTI void iupLineFileClose(IlineFile* line_file);

/** Reads a line from the file.
* Returns -1 if failed, otherwise returns the line size.
* \ingroup linefile */
IUP_EXPORTI int iupLineFileReadLine(IlineFile* line_file);

/** Returns the buffer used to read the line.
* \ingroup linefile */
IUP_EXPORTI const char* iupLineFileGetBuffer(IlineFile* line_file);

/** Returns a non zero value if reached the end of the file.
* \ingroup linefile */
IUP_EXPORTI int iupLineFileEOF(IlineFile* line_file);
